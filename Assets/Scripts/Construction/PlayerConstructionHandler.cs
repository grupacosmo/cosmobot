...
namespace Cosmobot
{
    public class PlayerConstructionHandler : MonoBehaviour, DefaultInputActions.IBuildingPlacementActions
    {
        ...
        [SerializeField]
        private ConstructionPreview constructionPreview;

        [SerializeField]
        private LayerMask buildTargetingCollisionMask;

        [SerializeField]
        private float maxBuildDistance = 20.0f;
        ...
        private int currentConstructionRotationSteps  ; // multiply by 90deg to get actual rotation
        private Vector3? currentPlacementPosition;
        private bool isPlacementActive  ;

        private Quaternion CurrentConstructionRotation =>
            Quaternion.Euler(0, currentConstructionRotationSteps * 90.0f, 0);
        ...
        public void OnStartPlacementTemp(InputAction.CallbackContext context)
        {
        ...
        private void ExecutePlacement()
        {
            ...
            GameObject newSite = Instantiate(constructionSitePrefab, (Vector3)currentPlacementPosition,
                CurrentConstructionRotation);
        ...
        private void ProcessPlacement()
        {
            ...
            currentPlacementPosition = ScanBuildingPlacement(snappedBuildPoint, currentBuildingInfo.GridSize,
                CurrentConstructionRotation);
        ...
        private Vector3 GetBuildPoint()
        {
            ...
            bool cameraRaySuccess = Physics.Raycast(cameraRay, out RaycastHit cameraRayHit, maxBuildDistance * 2,
                buildTargetingCollisionMask);

            if (cameraRaySuccess
                && Vector3.ProjectOnPlane(cameraTransform.position - cameraRayHit.point, Vector3.up).magnitude <
                maxBuildDistance)
            {
            ...
            Vector3 buildPointDistant = Vector3.ProjectOnPlane(cameraTransform.position, Vector3.up)
                                        + Vector3.ProjectOnPlane(cameraTransform.forward, Vector3.up) * maxBuildDistance;
        ...
        private Vector3 SnapToGrid(Vector3 vec, bool centerX, bool centerZ)
        {
            ...
            return newVec + new Vector3(centerX ? GlobalConstants.GRID_CELL_SIZE / 2.0f : 0, 0,
                centerZ ? GlobalConstants.GRID_CELL_SIZE / 2.0f : 0);
        }
            ...
            Quaternion buildingRotation)
        {
            ...
            Vector3 boxHalfExtents = new Vector3(buildingGridSize.x * GlobalConstants.GRID_CELL_SIZE * 0.5f, 1,
                buildingGridSize.y * GlobalConstants.GRID_CELL_SIZE * 0.5f);

            if (Physics.BoxCast(boxOrigin, boxHalfExtents, Vector3.down, out RaycastHit result, buildingRotation,
                    maxTerrainHeight * 2, buildTargetingCollisionMask))
            {
        ...
        private bool IsPlacementPositionValid()
        {
            ...
            Vector3 validCurrentPlacementPosition = new Vector3(currentPlacementPosition.Value.x,
                currentPlacementPosition.Value.y + 0.5f, currentPlacementPosition.Value.z);
            Ray objectRay = new Ray(validCurrentPlacementPosition, Vector3.down);
            bool objectRaySuccess =
